...
public class document_view extends JFrame {
	...
	public document_view() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(document_view.class.getResource("/xerox/Logo Transperent.png")));
		setResizable(false);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
				   ...
				   // 1. Get a connection to database
                    ...
                    sql= "UPDATE upload SET printed='"+status+"' WHERE id='"+temp1+"'";
                    stmt.executeUpdate(sql);
                    dispose();
                    update_balance ub=new update_balance();
                    ub.textField.setText(username);
                    ub.setVisible(true);
                 } catch (Exception exc) {
					  exc.printStackTrace();
		...
		lblByAniket.setBounds(446, 307, 124, 14);
		contentPane.add(lblByAniket);
		
		JLabel label = new JLabel("");
		label.setIcon(new ImageIcon(document_view.class.getResource("/xerox/back ground.jpeg")));
